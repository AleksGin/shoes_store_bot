...
class CrossworldService:
    ...
    async def send_rules(self, message: Message) -> Message:
        await self.message_service.send_message(
            message=message, text=Misc.instruction_text, keyboard=instruction_button()
        )
    ...
    async def open_main(
        ...
        if state:
            await state.clear()
        return await self.message_service.send_message(
            message=message, text=MainMenu.main_menu, keyboard=welcome_keyboard()
        )
    ...
    async def process_amount(self, message: Message, state: FSMContext) -> None:
        user_data = await state.get_data()
        clothe_name = user_data.get("clothe_name")

        if clothe_name is None:
            await self.message_service.send_message(message=message, text=Misc.wrong)
            return
            ...
            if message.text is not None:
                ...
                if amount > 0:
                    ...
                    calculating_message = await self.message_service.send_message(
                        message=message, text=Order.calculating_process_text
                    )
                    ...
                    await self.message_service.send_message(
                        ...
                        text=Order.total_amount.format(
                            int(result), delivery_price, comission
                        ),
                    )
                    await self.message_service.send_message(
                        message=message,
                    ...
                    await self.message_service.send_message(
                        message=message, text=Order.wrong_less_then_0_text
                    )
        except ValueError:
            await self.message_service.send_message(
                message=message, text=Order.wrong_value_text
            )
    ...
    async def not_make_order_action(
        ...
        await self.message_service.order_handle_callback(
            callback=callback, action=self.open_main, state=state
        )

    async def calculate_again_action(self, callback: CallbackQuery) -> None:
        await self.message_service.order_handle_callback(
            callback=callback, action=self.open_clothes_keyboard
        )
    ...
    async def callback_delivery_status(
        ...
        await self.message_service.order_handle_callback(
            callback=callback, action=self.delivery_status_action, state=state
        )

    async def delivery_status_action(self, message: Message, state: FSMContext) -> None:
    ...
    async def delivery_status_process(
        ...
        if message.text is not None and message.text.isdigit():
            ...
            search_message = await self.message_service.send_message(
                message=message, text=Order.search_order_text
            )
            ...
            if value == -1:
                ...
                await self.message_service.send_message(
                    message=message,
                    text=Order.order_number_status_text.format(order_number, value),
                )
                await self.message_service.send_message(
                    message=message,
            ...
            await self.message_service.send_message(
                message=message, text=Order.wrong_value_order_number_text
            )
